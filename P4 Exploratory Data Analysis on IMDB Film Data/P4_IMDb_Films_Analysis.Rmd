...
IMDb Film Data Analysis by Mohamed Malik
========================================================
...
imdb.2 <- subset(imdb, content_rating %in% c('G', 'GP', 'M', 'NC-17', 'PG', 'PG-13', 'R', 'X'))
...
ggplot(aes(x=num_voted_users), data=imdb.2) +
  geom_histogram(aes(color = I('black'), fill = I('#099DD9'))) +
  scale_x_log10(breaks = c(100, 1000, 10000, 50000, 150000, 1000000), labels = c(100, 1000, 10000, 50000, 100000, 1000000))
...
ggplot(aes(x=budget), data=imdb.2) +
  geom_histogram(aes(color = I('black'), fill = I('#099DD9'))) +
  scale_x_continuous(limit=c(min(imdb$budget)-1, quantile(imdb$budget, probs=0.99, na.rm=TRUE)))
...
ggplot(aes(x=budget), data=imdb.2) +
  geom_histogram(aes(color = I('black'), fill = I('#099DD9'))) +
  scale_x_log10(breaks = c(10000, 1000000, 20000000, 1000000000), labels = c(10000, 1000000, 20000000, 1000000000))
...
ggplot(aes(x=gross), data=imdb.2) +
  geom_histogram(aes(color = I('black'), fill = I('#099DD9'))) +
  scale_x_continuous(limit=c(0, quantile(imdb.2$gross, probs=0.99, na.rm = TRUE)))
...
ggplot(aes(x=gross), data=imdb.2) +
  geom_histogram(aes(color = I('black'), fill = I('#099DD9'))) +
  scale_x_log10(breaks = c(1000, 10000, 10000000, 60000000, 1000000000), labels = c(1000, 10000, 10000000, 60000000, 1000000000)) 
...
ggplot(aes(x=num_critic_for_reviews), data=imdb.2) +
  geom_histogram(aes(color = I('black'), fill = I('#099DD9'))) +
  scale_x_continuous(breaks=seq(0, quantile(imdb.2$num_critic_for_reviews, probs=0.99, na.rm=TRUE), 50), limit=c(0, quantile(imdb.2$num_critic_for_reviews, probs=0.99, na.rm=TRUE)))

```

After removing the top .1% of data, we have a positively skewed distribution of number of reviews from critics. The median is 110 reviews.
...
ggplot(aes(x=duration), data=imdb.2) +
  geom_histogram(aes(color = I('black'), fill = I('#099DD9'))) +
  scale_x_continuous(breaks=seq(0, quantile(imdb.2$duration, probs=0.99, na.rm = TRUE), 10), limit=c(0, quantile(imdb.2$duration, probs=0.99, na.rm = TRUE)))

```

Removed the top 1% of data here as well. No log transformation here needed - we have a normal distribution that gives a clear idea of the distribution of running times. The median duration is 104 minutes.
...
correlations <- cor(imdb.2[,c('imdb_score', 'director_facebook_likes', 'cast_total_facebook_likes', 'num_critic_for_reviews', 'duration', 'num_voted_users', 'actor_1_facebook_likes', 'actor_2_facebook_likes', 'actor_3_facebook_likes', 'movie_facebook_likes', 'facenumber_in_poster', 'title_year', 'gross', 'budget')], use="pairwise.complete.obs")
...
The negative correlation between imdb_score and title_year that we saw in the corr plot is shown here. As we go forward in time, the median IMDb score appears to drop slightly. Again, this could be due to the fact there are more movies being produced now then there 20+ years. Thus, we're seeing greater instances of bad to average films, and as a result a lower IMDb score median. The downward trend of the line always captures the negative (but weak) correlation.

Let's visualize the relationship between content ratings and IMDb scores and see if there's a pattern.

```{r echo=FALSE, message=FALSE, warning=FALSE}

content_ratings_medians <- reorder(imdb.2$content_rating, -imdb.2$imdb_score, median)
...
ggplot(aes(x=budget, y = imdb_score), data = imdb.2) +
  geom_point(alpha=1/5) +
  scale_x_continuous(limits = c(0, quantile(imdb.2$budget, probs=0.99, na.rm = TRUE))) +
  geom_smooth()
...
ggplot(aes(x=gross, y = imdb_score), data = imdb.2) +
  geom_point(alpha=1/5) +
  scale_x_continuous(limits = c(0, quantile(imdb.2$gross, probs=0.99, na.rm = TRUE))) + 
  geom_smooth()
...
We know that duration and number of votes users are positively correlated (0.35), so let's see how adding in duration as color to the num_voted_use/imdb_score plot looks.

```{r echo=FALSE, message=FALSE, warning=FALSE}

#  scale_colour_gradientn(colours=rainbow(4)) adds color

#Num_Voted_Users / Duration

ggplot(aes(x=num_voted_users, y = imdb_score), data = subset(imdb.2, duration > quantile(duration, 0.01, na.rm=TRUE) & duration < quantile(duration, 0.95, na.rm = TRUE) )) +
  geom_point(aes(color=duration)) +
  scale_x_continuous(limits = c(0, quantile(imdb.2$num_voted_users, probs=0.99, na.rm = TRUE))) +
  scale_colour_gradientn(colours=rainbow(4))


```

We can see their correlation here - duration generally increases with number of users who have voted. Additionally, you can see the correlation between duration and imdb_scores.
...
ggplot(aes(x=num_voted_users, y = imdb_score), data = subset(imdb.2, movie_facebook_likes <= quantile(movie_facebook_likes, 0.95))) +
...
ggplot(aes(x=num_critic_for_reviews, y = imdb_score), data = subset(imdb.2, gross <= quantile(gross, 0.95, na.rm = TRUE))) +
  geom_point(aes(color=gross)) +
  scale_x_continuous(limits = c(0, quantile(imdb.2$num_critic_for_reviews, probs=0.99, na.rm = TRUE))) +
  scale_colour_gradientn(colours=rainbow(4))
...
ggplot(aes(x=num_voted_users, y = imdb_score), data = subset(imdb.2, num_voted_users < quantile(num_voted_users, probs = 0.99, na.rm=TRUE))) +
  geom_point(alpha=1/5) +
  geom_smooth() +
...
stat_summary(fun.y = mean, geom = 'line', shape = 4) +
  facet_wrap(~ content_rating) +
  labs(y = "IMDb Score", x = "Year of Release") +
  ggtitle("IMDb Score vs. Year of Release By Content Rating") +
  theme(plot.title = element_text(hjust = 0.5))
